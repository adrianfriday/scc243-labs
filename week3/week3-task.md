...
1. You may get differences in estimates, or only be able to get an estimate for a device that's as similar as possible to the one you have.  Try to sense check what you find, web searches/AI summaries are notorious for giving you spurious results!
...
2. Look up the carbon footprint of the device's manufacture, if possible you may be able to identify and remove the use phase emissions
...
5. Now add a row comparing the annual use energy footprint with the embodied footprint per year (i.e. calculate the 'footprint per year' of embodied CO<sub>2</sub>e by dividing it by the number of years you keep the device).
6. You should add a row for different lifetimes 1, 3, 5 and 10 years, showing the cumulative energy from use emissions vs. the footprint per year.  A crossing point where use overtakes embodied might be evidence for some devices.  In reality, some devices have shorter lifespans than this, but many (e.g. TVs, ovens) could last much longer.
